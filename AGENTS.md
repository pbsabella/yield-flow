...
- **Yield calc:** core math in `lib/domain/yield-engine.ts`. Summaries are built by delegating modules (`interest.ts`, `accrued-interest.ts`, `rollover.ts`, `cashflow.ts`, `ai-context.ts`). Never duplicate or fork the math.
