...
export default function StatsGrid({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      <div className="border-border-subtle bg-surface-base rounded-2xl border px-5 py-4 transition-colors duration-200 ease-out">
        <p className="text-muted-foreground text-xs font-medium">Total Principal</p>
        <p className="font-financial mt-3 text-2xl font-semibold">
          {formatPhpCurrency(totalPrincipal)}
        </p>
        <p className="text-muted-foreground mt-2 text-xs">
          Excludes settled investments.
        </p>
      </div>
      <div className="border-border-subtle bg-surface-base rounded-2xl border px-5 py-4 transition-colors duration-200 ease-out">
        <p className="text-muted-foreground text-xs font-medium">Income This Month</p>
        <p className="text-muted-foreground mt-1 text-xs">
          Net interest · {currentMonthLabel}
        </p>
        <p className="font-financial mt-3 text-2xl font-semibold">
        ...
        {currentMonthIncomePending > 0 || currentMonthIncomeSettled > 0 ? (
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
            {currentMonthIncomePending > 0 ? (
              <span className="bg-status-warning-bg text-status-warning-fg ring-status-warning-border inline-flex items-center rounded-full px-2.5 py-1 ring-1">
                {formatPhpCurrency(currentMonthIncomePending)} pending
              </span>
            ) : null}
            {currentMonthIncomeSettled > 0 ? (
              <span className="bg-status-success-bg text-status-success-fg ring-status-success-border inline-flex items-center rounded-full px-2.5 py-1 ring-1">
      ...
      <div className="border-border-subtle bg-surface-base rounded-2xl border px-5 py-4 transition-colors duration-200 ease-out">
        <p className="text-muted-foreground text-xs font-medium">Next Maturity</p>
        {nextMaturity ? (
          <>
            ...
            <p className="font-financial text-bank-name-fg mt-2 text-base font-semibold">
